...
let max_cheese matrika =
    let dimx = Array.length matrika in
    let dimy = Array.length matrika.(0) in
      let rec best_path x y =
        let trenutna = matrika.(x).(y) in
        let gremo_desno = if (x + 1 = dimx) then 0 else best_path (x + 1) y in
        let gremo_dol = if (y + 1 = dimy) then 0 else best_path x (y + 1) in
        trenutna + max gremo_desno gremo_dol
      in
      best_path 0 0
    
let max_cheese_bottom matrika = 
      let bottom = List.init (Array.length matrika.(0)) (fun _ -> 0) in
      let lst = List.rev (List.map Array.to_list (Array.to_list matrika)) in
